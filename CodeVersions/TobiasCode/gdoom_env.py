...
class GDoomEnv(gym.Env):
    ...
    def shape_reward(self, r_t, misc, prev_misc, t=None):
        ...
        if self.level==2: # For 'defend_the_center.cfg':
            # Check any kill count
            #if (misc[0] > prev_misc[0]):
                #r_t = r_t + 2 # Maybe 2 better
            ...
            if (misc[2] < prev_misc[2]):  # Loss HEALTH
                r_t = r_t - 0.1
            #Maybe a living reward as just
            r_t=r_t+0.05
            return r_t
        
        if self.level==6: # For 'predict_poistion.cfg':
            # Check any kill count
            if (misc[0] > prev_misc[0]):
                r_t = r_t + 2 # Maybe 2 better
...
#     env_cpu = gym.make("doom_scenario0_64-v0")
#     frame = env_cpu.reset()
#     print("Frame size for cpu player: ", np.asarray(frame).shape )
#     print(env_cpu.step(1))
#     #One
#     timealive=0;
#     kills=0;
#     N=100;
...
#             #print(env_cpu.step(1.0))
#             choiceofaction=int(np.random.choice([0,1,2]))
#             action=env_cpu.step(choiceofaction)
#             _,_,is_finished,d=action
#             i=i+1;
#             #print(is_finished,is_finished==True,~(is_finished==True))
#             #print(is_finished)
#         if np.mod(k,10)==0:
#             print(k)
#         kills=kills+d['kills']
#         timealive=timealive+d['time_alive'] 
#         env_cpu.reset()
#     kills=kills/N
#    timealive=timealive/N
#    print('Kills:',kills,' Timealive:',timealive)
